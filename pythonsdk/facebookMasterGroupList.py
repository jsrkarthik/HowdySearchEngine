...
fo = open("complete_search_group2.txt", "wb+")
fo.close()
fo = open("complete_search_group2.txt","a+")
categoryList = set()
pageList = []
fo1 = open("CompleteGroupList.txt","wb+")
fo_pageInfo = open("completeGroupInfo.txt","wb+")
for query in queryList :
	graphQueryStr = '/search?q=\"'+query+'\"&type=group'
	results = graph.get(graphQueryStr)
	while(len(results['data'])>0 ) :
		for result in results['data'] :
			page_result = graph.get(result['id'])
			if(page_result["privacy"]=="OPEN") :
				temp = []
...
fo_pageInfo.close()
fieldList = set()
print "len of groupList = ",len(pageList)	
count = 0
